...
test('POST validation response based on componentTypeId', async ({ request }) => {
  ...
  const vrRes = await request.get('http://verifi-nlbC3-n2OMroH9cKeE-92f8f4467dd39713.elb.ap-south-1.amazonaws.com/workflow/NDOH-250917-1331E');
  ...
  for (const item of claimItems) {
    ...
    if (componentTypeId === "ComponentType@identityBasicDetails") {
      const identityPayload = {
        ...
        identityPostRes = await request.post(
...
// Step 5: PUT request to update state to CLOSED for LOA
...
//change the state to closed using PUT API for Identity component
// ✅ Extract the JSON from identityPostRes
const identityPostJson = await identityPostRes.json();
const identityValidationResponseId = identityPostJson.entities[0].id;

// ✅ Now send the PUT request using the real ID
const identityPutResponse = await request.put(
  `http://verifi-nlbC3-fdG3aGSbkH7t-b99f8e0e7ddd5c6b.elb.ap-south-1.amazonaws.com/validation-response/${identityValidationResponseId}`,
  {
    headers: {
      'Content-Type': 'application/json'
    },
...
console.log(`PUT validation-response status: ${identityPutResponse.status()}`);
expect(identityPutResponse.status()).toBe(200);
console.log(identityPutResponse.json());

//change the state to closed using PUT API for Education component

const eduPostJson = await eduPostRes.json();
const eduValidationResponseId = eduPostJson.entities[0].id;
const eduPutResponse = await request.put(
  `http://verifi-nlbC3-fdG3aGSbkH7t-b99f8e0e7ddd5c6b.elb.ap-south-1.amazonaws.com/validation-response/${eduValidationResponseId}`,
  {
    headers: {
      'Content-Type': 'application/json'
    },
    ...
    console.log(`PUT validation-response for education status: ${eduPutResponse.status()}`);
    expect(eduPutResponse.status()).toBe(200);
    console.log(await eduPutResponse.json());
