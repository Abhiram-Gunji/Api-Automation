...
const verificationRequestId = 'VerificationRequest@a4b550d3-c816-45fb-9953-aea2daa9a76a';
//Get Verification request payment by vrid
test('Check verification request payment by ID', async ({ request }) => {
  
  const url = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request-payment/index/${verificationRequestId}`;
...
const url= `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request/${verificationRequestId}`;
const response = await request.get(url);
const data= await response.json();
  expect(response.status()).toBe(200);
  console.log('Response data:', JSON.stringify(data, null, 2));
...
const getUrl = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request-payment/index/${verificationRequestId}`;
  ...
  const putUrl = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request-payment/${verificationRequestId}`;
  const payload = {
...
test('Update all payment contracts to SUCCESS', async ({ request }) => {
  ...
  const fetchUrl = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request-payment/index/${verificationRequestId}`;
  ...
  for (const contractId of contracts) {
    const updateUrl = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/core-verification-data-structure-service/verification-request-payment-contracts/${contractId}/?updateType=ADD`;
...
test('Trigger pre-validation-integrity-check API', async () => {
  const request = await baseRequest.newContext();
    const url = `http://nginx--nlbc3-fo19n4q4lzrb-761f631e132758db.elb.ap-south-1.amazonaws.com/verification-request/verification-request/${verificationRequestId}/pre-validation-integrity-check`;
